...
MACRO(KOKKOS_MAKE_LIBKOKKOS)
  ...
  IF(${CMAKE_VERSION} VERSION_LESS "3.12" OR MSVC)
    ...
    ADD_LIBRARY(${KOKKOS_LIBRARY_NAME} ${KOKKOS_SOURCE_DIR}/core/src/dummy.cpp
      $<TARGET_OBJECTS:${KOKKOS_CORE_NAME}>
      $<TARGET_OBJECTS:${KOKKOS_CONTAINERS_NAME}>
    )
    TARGET_LINK_LIBRARIES(${KOKKOS_LIBRARY_NAME} PUBLIC ${KOKKOS_LINK_OPTIONS})
    #still need the header-only library
    TARGET_LINK_LIBRARIES(${KOKKOS_LIBRARY_NAME} PUBLIC ${KOKKOS_ALGORITHMS_NAME})
    KOKKOS_LINK_TPLS(${KOKKOS_LIBRARY_NAME})
    #these properties do not work transitively correctly so we
    #need some verbose hackery to make it work
    GET_TARGET_PROPERTY(CORE_DIRS ${KOKKOS_CORE_NAME}       INTERFACE_INCLUDE_DIRECTORIES)
    GET_TARGET_PROPERTY(CTRS_DIRS ${KOKKOS_CONTAINERS_NAME} INTERFACE_INCLUDE_DIRECTORIES)
    TARGET_INCLUDE_DIRECTORIES(${KOKKOS_LIBRARY_NAME} PUBLIC ${CORE_DIRS})
    TARGET_INCLUDE_DIRECTORIES(${KOKKOS_LIBRARY_NAME} PUBLIC ${CTRS_DIRS})

    GET_TARGET_PROPERTY(CORE_FLAGS ${KOKKOS_CORE_NAME}       INTERFACE_COMPILE_OPTIONS)
    GET_TARGET_PROPERTY(CTRS_FLAGS ${KOKKOS_CONTAINERS_NAME} INTERFACE_COMPILE_OPTIONS)
    TARGET_COMPILE_OPTIONS(${KOKKOS_LIBRARY_NAME} PUBLIC ${CORE_FLAGS})
    TARGET_COMPILE_OPTIONS(${KOKKOS_LIBRARY_NAME} PUBLIC ${CTRS_FLAGS})
  ...
  ELSE()
    ADD_LIBRARY(${KOKKOS_LIBRARY_NAME} ${KOKKOS_SOURCE_DIR}/core/src/dummy.cpp)
    TARGET_LINK_LIBRARIES(${KOKKOS_LIBRARY_NAME} PUBLIC ${KOKKOS_CORE_NAME} ${KOKKOS_CONTAINERS_NAME})
    TARGET_LINK_LIBRARIES(${KOKKOS_LIBRARY_NAME} PUBLIC ${KOKKOS_ALGORITHMS_NAME})
  ENDIF()
  SET_TARGET_PROPERTIES(${KOKKOS_LIBRARY_NAME} PROPERTIES VERSION ${Kokkos_VERSION})
  KOKKOS_INTERNAL_ADD_LIBRARY_INSTALL(${KOKKOS_LIBRARY_NAME})
ENDMACRO()
